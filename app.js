...
const CloudSync = {
    ...
    /** POST to GAS using form submission (avoids redirect losing body) */
    _postToGAS(url, payload) {
        return new Promise((resolve) => {
            const iframe = document.createElement('iframe');
            iframe.name = 'gas_sync_frame_' + Date.now();
            ...
            // GAS reads e.parameter for form data, and e.postData for raw body
            // We send as a hidden field that GAS can parse
    ...
    async _push(key, data) {
        try {
            const url = API.getUrl();
            await this._postToGAS(url, { action: 'saveSync', [key]: data });
    ...
    async saveAll() {
        ...
        try {
            ...
            await this._postToGAS(url, payload);
            return true;
        } catch (e) {
